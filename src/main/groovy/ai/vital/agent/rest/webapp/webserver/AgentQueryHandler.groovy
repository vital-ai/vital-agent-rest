...
class AgentQueryHandler implements Handler<RoutingContext> {
	
	private final static Logger log = LoggerFactory.getLogger(AgentQueryHandler.class)
	
	static JsonSlurper jsonParser = new JsonSlurper()
		
	static Vertx vertxInstance
		
	public AgentQueryHandler(Vertx vertxInstance) {
	...
	public void handle(RoutingContext ctx) {
		...
		ctx.request().bodyHandler({ body ->
			...
			log.info("AgentQueryHandler: " + body)
			...
			try {
				...
				vertxInstance.eventBus().send("agentquery", requestJSON, options) { Future<Message> response ->
					...
					if(response.succeeded()) {
						...
						String message = 'error in agent query.'
				...
				String message = 'AgentQueryHandle Error: ' + ex.localizedMessage
